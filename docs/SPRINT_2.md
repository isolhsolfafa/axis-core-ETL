...
> 시작일: 2026-03-10
> 상태: 준비 중 (Task 5~6 VIEW 선행 개발 완료, Task 1~4 내일 시작)
...
## Task 1: DB 스키마 — etl.change_log 테이블
...
**실행 위치**: Railway PostgreSQL 콘솔 또는 마이그레이션 스크립트

---

## Task 2: step2_load.py 수정 — change_log 기록 로직
...
## Task 3: 출고일자(실적) 적재 + shipped 상태 처리
...
## Task 4: OPS BE 엔드포인트 — change_log 조회 API
...
## Task 5: AXIS-VIEW — ETL 변경 이력 페이지 ✅ 선행 개발 완료 (Mock)

> QR관리 하위 페이지로 구현 완료 (2026-03-09). OPS BE API 연동만 남음.
...
import { fetchEtlChanges } from '../api/etl'
// GET /api/admin/etl/changes?days=7&field=&serial_number=&limit=100
```
...
Task 1 (DB 스키마)
  ↓
Task 2 (step2 수정)  ←──── Task 3 (출고일자, 현장 확인 후)
  ↓
Task 4 (OPS BE 엔드포인트)
  ↓
Task 5 (VIEW Mock → API 연동)   ← ✅ Mock 선행 완료
  ↓
Task 6 (Sidebar 메뉴)           ← ✅ 구현 완료
```

- Task 1~2: ETL 단독 진행 가능 ← **내일(3/10) 여기서 시작**
- Task 3: Excel 출고일자 컬럼 현장 확인 후 진행
- Task 4: OPS BE 요청 (OPS_API_REQUESTS.md 등록 완료)
- Task 5: Mock → API 전환만 남음 (Task 4 완료 후)
- Task 6: ✅ 완료
